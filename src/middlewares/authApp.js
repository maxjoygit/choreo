...
const authApp = async (req, res, next) => {
  ...
  if (!authHeader)
    return res.json({
      error: true,
      data: 'Authorization header not available!',
    });

  if (!authHeader.startsWith('Bearer'))
    return res.json({
      error: true,
      data: 'Not A Beader authorization header!',
    });
  ...
  try {
    ...
    const [rows] = await sql.query(
      'SELECT * FROM apitokens WHERE apitoken = ? AND status = ? AND deleted_at IS NULL',
      [encryptedToken, 1],
    );

    if (rows?.length === 0)
      return res.json({ error: true, data: 'Invalid api key!' });
    ...
    req.user_id = payload?.data[0]?.user_id
    ...
    req.roles = payload?.data[0]?.roles
      ? JSON.parse(payload?.data[0]?.roles)
      : [];
